...
public class Fenetre implements ActionListener {
	/** Cette classe s'occupe de la gestion du placement des différents objets ainsi que des actions exécutées lorsque les boutons seront appuyés. */
	JFrame fenetre;
	JLabel chfL, euroL;
	...
	Fenetre() {
		...
		chfTF.setBounds(33,6,85,20);
		JTextField euroTF = new JTextField();
		euroTF.setBounds(306,6,85,20);
		
		// BOUTONS RADIO
		JRadioButton chfToEuroRB = new JRadioButton("CHF => €");
		chfToEuroRB.setBounds(135,0,75,38);
		JRadioButton euroToChfRB = new JRadioButton("€ => CHF");
		euroToChfRB.setBounds(215,0,75,38);
		// On place les deux boutons dans un groupe afin de pouvoir choisir entre l'un et l'autre au sein du même groupe.
